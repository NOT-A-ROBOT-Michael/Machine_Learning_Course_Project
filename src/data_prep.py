...
def dataprep(filepath):
    ...
    df[skewed_features] = np.log1p(clean_encoded_featured_df[skewed_features])

    clean_encoded_featured_transformed_df = clean_encoded_featured_df.copy()
    
    store_filepath = filepath + '/data/train_data.csv'
    
    clean_encoded_featured_transformed_df.to_csv(filepath, index=True)
